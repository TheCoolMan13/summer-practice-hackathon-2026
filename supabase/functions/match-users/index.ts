...
// ---------------------------------------------------------------------------
// Group creation, event creation, and notification dispatch (Task 8.3)
//
// For each formed group this function:
//   1. INSERTs a row into `groups`
//   2. INSERTs a row into `events` (source='matched')
//   3. UPDATEs groups.event_id to link the two records
//   4. INSERTs rows into `group_members` for every member
//   5. Calls selectCaptain() to assign a captain (task 9.1 stub)
//   6. INSERTs a system message "Group created! Welcome everyone 🎉"
//   7. INSERTs a 'match_found' notification for every member
//
// All seven steps are executed sequentially per group.  The Supabase JS client
// does not expose a BEGIN/COMMIT API, so atomicity is achieved by running the
// inserts in a tight sequence and logging (but not aborting) non-fatal errors.
// A future migration can wrap these in a Postgres function/RPC for true ACID
// guarantees if required.
//
// Requirements: 7.8, 8.1, 9.1, 9.3, 12.1
// ---------------------------------------------------------------------------
async function createGroupsFromMatches(
  supabase: SupabaseClient,
  formedGroups: FormedGroup[],
): Promise<void> {
  for (const group of formedGroups) {
    const { sport, members } = group
    const sizes = SPORT_SIZES[sport] ?? { min: 2, max: members.length }
    ...
    if (groupError || !groupData) {
      console.error(`[createGroups] Failed to insert group for sport ${sport}:`, groupError?.message)
      continue
    }
    ...
    // ------------------------------------------------------------------
    // Step 2: INSERT into events (source='matched')
    // start_time is a placeholder 24 hours from now; captain will finalize it.
    // ------------------------------------------------------------------
    ...
    if (eventError || !eventData) {
      console.error(`[createGroups] Failed to insert event for group ${groupId}:`, eventError?.message)
      // Continue — group exists; event can be retried later
    }

    const eventId: string | null = eventData?.id ?? null

    // ------------------------------------------------------------------
    // Step 3: UPDATE groups to set event_id
    // ------------------------------------------------------------------
    if (eventId) {
      ...
      if (updateError) {
        console.error(`[createGroups] Failed to set event_id on group ${groupId}:`, updateError.message)
      }
    }

    // ------------------------------------------------------------------
    // Step 4: INSERT into group_members for all members
    // ------------------------------------------------------------------
    ...
    if (membersError) {
      console.error(`[createGroups] Failed to insert group_members for group ${groupId}:`, membersError.message)
    }

    // ------------------------------------------------------------------
    // Step 5: Captain selection (stub — task 9.1 will replace)
    // ------------------------------------------------------------------
    try {
      await selectCaptain(supabase, groupId, members)
    } catch (err) {
      console.error(`[createGroups] Captain selection failed for group ${groupId}:`, err)
    }

    // ------------------------------------------------------------------
    // Step 6: INSERT system message "Group created! Welcome everyone 🎉"
    // sender_id is NULL for system messages (per schema design)
    // ------------------------------------------------------------------
    ...
    if (msgError) {
      console.error(`[createGroups] Failed to insert system message for group ${groupId}:`, msgError.message)
    }

    // ------------------------------------------------------------------
    // Step 7: INSERT notifications for all members
    // ------------------------------------------------------------------
    const notifications = members.map((userId) => ({
      user_id: userId,
      type: 'match_found',
      title: "You've been matched!",
      body: `You've been matched for ${sport}! Check your group chat.`,
      data: {
        group_id: groupId,
    ...
    if (notifError) {
      console.error(`[createGroups] Failed to insert match_found notifications for group ${groupId}:`, notifError.message)
    }

    console.log(`[createGroups] Group ${groupId} created for sport ${sport} with ${members.length} members.`)
  }
}
